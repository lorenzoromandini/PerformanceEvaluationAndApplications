...
def computation(trace, i):
    ...
    beta = SX4 - 3
    print(
        f"Standard deviation = {sigma}, Coefficient of Variation = {c_v}, Excess Kurtosis = {beta}"
    )
    ...
    print(
        f"Median = {q2}, quartile_1 = {q1}, quartile_3 = {q3}, percentile_5 = {p05}, percentile_90 = {p90}"
    )
...
def calculate_percentile(trace, P):
    ...
    if h == N:
        ...
        h_int = np.floor(h)
        return trace[int(h_int)] + (h - h_int) * (
            trace[int(h_int) + 1] - trace[int(h_int)]
        )
